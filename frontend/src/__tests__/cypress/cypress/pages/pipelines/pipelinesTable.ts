...
class PipelinesTable {
  ...
  shouldSortTable({
    ...
    const sortPipelineByAscending = (): void => {
      cy.intercept(
        'GET',
        `**/api/service/pipelines/${projectName}/dspa/apis/v2beta1/pipelines?sort_by=name+asc**`,
      ).as('asc');
      this.findTableHeaderButton('Pipeline').click();
      this.findTableHeaderButton('Pipeline').should(be.sortAscending);
      cy.wait('@asc').should('exist');
    };

    const sortPipelineByDescending = (): void => {
      cy.intercept(
        'GET',
        `**/api/service/pipelines/${projectName}/dspa/apis/v2beta1/pipelines?sort_by=name+desc**`,
      ).as('descend');
      this.findTableHeaderButton('Pipeline').click();
      this.findTableHeaderButton('Pipeline').should(be.sortDescending);
      cy.wait('@descend').should('exist');
    };

    const sortPipelineCreationByAscending = (): void => {
      cy.intercept(
        'GET',
        `**/api/service/pipelines/${projectName}/dspa/apis/v2beta1/pipelines?sort_by=created_at+asc**`,
      ).as('asc');
